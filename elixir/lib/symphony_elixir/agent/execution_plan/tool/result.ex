defmodule SymphonyElixir.Agent.ExecutionPlan.Tool.Result do
  @moduledoc """
  Result envelopes and bounded summaries for generic Agent execution-plan tools.
  """
  ...
  def changed_items(before_plan, updated_plan) do
    before_by_id =
      ...
      |> Map.new(&{Map.get(&1, Fields.item_id()), &1})

    updated_plan
    |> Map.get(Fields.items(), [])
    |> Enum.reject(fn item -> Map.get(before_by_id, Map.get(item, Fields.item_id())) == item end)
  end

  @spec created_items(map()) :: [map()]
  def created_items(plan) when is_map(plan), do: Map.get(plan, Fields.items(), [])
  ...
  defp item_summary(item) do
    evidence_refs = Map.get(item, Fields.evidence_refs(), [])

    item
    |> Map.take(@summary_item_keys)
    |> Map.put(ToolContract.evidence_ref_count_key(), length(evidence_refs))
    |> Map.put(ToolContract.evidence_kinds_key(), evidence_refs |> Enum.map(&Map.get(&1, Fields.evidence_kind())) |> Enum.reject(&is_nil/1) |> Enum.uniq())
  end
  ...
  defp typed_error({:unsupported_tool, tool}) do
    {ToolErrorCodes.unsupported_tool(), "Agent execution plan tool is not supported.", %{Metadata.Contract.tool() => tool}}
  end

  defp typed_error(%{code: code, message: message} = error), do: {to_string(code), message, Map.delete(error, :message)}

  defp typed_error(reason) do
    {ToolErrorCodes.tool_failed(), "Agent execution plan tool execution failed.", %{ToolContract.reason_key() => inspect(reason)}}
  end
end
