defmodule SymphonyElixir.Agent.Runtime.WorkerDaemon.Client.Filters do
  ...
  def session_filters(%Target{} = target, opts) do
    ...
    %{
      "owner" => Keyword.get(opts, :worker_daemon_owner, "symphony"),
      "tenant_id" => Keyword.get(opts, :tenant_id) || Connection.metadata_value(target.metadata, :tenant_id),
      "run_id" => Keyword.get(opts, :run_id) || Connection.metadata_value(target.metadata, :run_id),
  ...
  defp normalize_filter_key(key) when key in ["owner", :owner], do: "owner"
  defp normalize_filter_key(key) when key in ["tenant_id", :tenant_id], do: "tenant_id"
  defp normalize_filter_key(key) when key in ["run_id", :run_id], do: "run_id"
  defp normalize_filter_key(key) when key in ["status", :status], do: "status"
