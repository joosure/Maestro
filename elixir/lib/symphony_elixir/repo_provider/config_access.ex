defmodule SymphonyElixir.RepoProvider.ConfigAccess do
  @moduledoc false
  ...
  @doc """
  Returns `true` when `value` is a non-empty binary string.
  """
  ...
  @doc """
  Extracts a named sub-map from the repo config's `provider` section.
  ...
  ## Examples
      ...
      iex> provider_field(%{provider: %{"options" => %{"required_pr_label" => "ok"}}}, "options")
      %{"required_pr_label" => "ok"}

      iex> provider_field(%{provider: %{}}, "options")
      %{}
  """
  @spec provider_field(map(), String.t()) :: map()
  def provider_field(repo, field_name) when is_map(repo) and is_binary(field_name) do
