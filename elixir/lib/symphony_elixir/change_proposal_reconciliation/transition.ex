defmodule SymphonyElixir.ChangeProposalReconciliation.Transition do
  ...
  defp transition_issue(settings, issue, state, context, route_facts, facts, decision, opts) do
    target_state = RoutePolicy.raw_state_for_route_key(context.raw_state_by_route_key, decision.target_route)

    cond do
      is_nil(target_state) ->
        Events.transition(settings, :warning, Contract.event(:transition_failed), issue, state, route_facts, facts, decision, %{
          error: inspect({:missing_raw_state_for_route_key, decision.target_route})
        })

        state

      route_facts.route_key == decision.target_route ->
      ...
      true ->
        confirm_and_update_issue(settings, issue, state, route_facts, facts, decision, target_state, opts)
    end
  end

  defp confirm_and_update_issue(settings, issue, state, route_facts, facts, decision, target_state, opts) do
    ...
    with {:ok, %Issue{} = refreshed_issue} <- fetch_single_issue(issue.id, opts),
         ...
         :ok <- confirm_target_route(settings, confirmed_issue, decision.target_route) do
      Events.transition(
        settings,
        :info,
  ...
  defp confirm_target_route(settings, %Issue{} = issue, expected_route) do
    case RouteContext.route_facts(issue, RouteContext.for_issue(settings, issue)) do
      %RouteFacts{route_key: ^expected_route} -> :ok
      %RouteFacts{} = route_facts -> {:error, {:target_route_unconfirmed, route_facts.route_key, expected_route}}
      nil -> {:error, :target_route_unresolved}
    end
  end
