defmodule SymphonyElixir.Workflow.Runtime.Store do
  @moduledoc """
  ...
  alias SymphonyElixir.Observability.WorkflowConfig, as: ObservabilityWorkflowConfig
  ...
  defp load_state(path) do
    with {:ok, workflow} <- Workflow.load(path),
      ...
      ObservabilityWorkflowConfig.configure_from_workflow(path, workflow, workflow_event_fields(path, nil))
      ObsLogger.emit(:info, :workflow_loaded, workflow_event_fields(path, stamp))
  ...
  defp log_reload_error(path, reason) do
    ObsLogger.emit(
      ...
      workflow_event_fields(path, nil, %{error: inspect(reason)})
    )
  end

  defp workflow_event_fields(path, stamp, extra \\ %{}) do
    %{
      component: "workflow_store",
      workflow_path: path,
      workflow_hash: workflow_hash(stamp)
    }
    |> Map.merge(extra)
  end

  defp workflow_hash({_mtime, _size, hash}), do: hash
  defp workflow_hash(_stamp), do: nil
end
