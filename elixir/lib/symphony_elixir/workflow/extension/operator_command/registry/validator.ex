defmodule SymphonyElixir.Workflow.Extension.OperatorCommand.Registry.Validator do
  @moduledoc false
    ...
    duplicates =
      ...
      |> Enum.map(fn {module, specs} ->
        %{
          ...
          sources: Enum.map(specs, &Error.source_diagnostic(&1.source))
        }
      end)

    case duplicates do
      [] -> :ok
      duplicates -> {:error, Error.invalid(:duplicate_operator_command_modules, duplicates: duplicates)}
    end
  end
  ...
  def unique_ids(entries) do
    ...
    case duplicates do
      [] -> :ok
      duplicates -> {:error, Error.invalid(:duplicate_operator_command_ids, duplicates: duplicates)}
    end
  end
end
