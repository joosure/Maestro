defmodule SymphonyElixir.Workflow.Extensions.CodingPrDelivery.Reconciliation.RouteContext do
  @moduledoc false
    ...
    %{
      ...
      policy_by_route_key:
        IssueContext.policy_by_route_key(
          issue,
          RouteContextDefaults.default_policy_by_route_key(profile_context)
        )
    }
  end

  defp global_route_context(settings) do
    profile_context = RouteContextDefaults.resolve_profile!(settings.workflow.profile)

    %{
      profile_context: profile_context,
      state_phase_map: RouteContextDefaults.state_phase_map(settings.tracker) || %{},
  ...
  defp configured_raw_state_route_maps(settings) do
    ...
    workflows_by_type = RouteContextDefaults.workflows_by_type(tracker)
  ...
  defp raw_state_by_route_key_from_workflow(workflow, base_map, profile_context) when is_map(workflow) do
    workflow
    |> RouteContextDefaults.workflow_raw_state_by_route_key()
  ...
  defp global_raw_state_by_route_key(tracker, profile_context) do
    tracker
    |> RouteContextDefaults.raw_state_by_route_key()
  ...
  defp global_policy_by_route_key(tracker, profile_context) do
    default_policy_by_route_key = RouteContextDefaults.default_policy_by_route_key(profile_context)

    tracker
    |> RouteContextDefaults.policy_by_route_key()
