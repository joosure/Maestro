...
workflow:
  ...
  reconciliation:
    change_proposal:
      enabled: true
      candidates:
        # Normal cycles reconcile only runtime-known PR targets. On startup,
        # Symphony does one bounded source-route scan to recover review backlog
        # candidates that existed before this process started.
...
3. Use the generated Typed Workflow Tool Inventory plus the bundled TAPD and repo skills for how to perform those actions. Do not invent direct TAPD/CNB/API fallbacks outside those documented capability boundaries.
...
6. Create or update the CNB PR through inventory-listed typed tools.
...
- `SOURCE_REPO_PROVIDER_REPOSITORY` is optional for standard CNB clone URLs because Symphony infers the CNB repository path from `SOURCE_REPO_URL`. Set it only when using a mirror, non-standard URL, or an explicit override; when set, use the CNB repository path such as `owner/group/repo`, not the full clone URL.
- The CodeBuddy Code CLI must be available as `codebuddy`, `model: glm-5.1` is the template-pinned model, and `credential://codebuddy_code/default` must be present in the Symphony agent credentials store. Symphony verifies the configured model against CodeBuddy session model metadata during startup. Plugin-hosted tools, auxiliary HTTP, usage metrics, quota probing, and remote runtime are intentionally not enabled by this template.
...
Use the exact CodeBuddy-facing callable names listed in the generated inventory
below. The inventory is the source for provider-specific callable names; the
bundled skills define typed capability semantics and argument shape. If a
required typed tool is missing, stop as blocked, record the blocker in the TAPD
...
<!-- symphony-include: _partials/repo_provider/cnb_change_proposal_notes.md -->
...
Route facts are authoritative:

- `policy_by_route_key` defines behavior; `raw_state_by_route_key` is the TAPD status lookup used for reads and writes.
- `state_phase_map` maps raw TAPD statuses to shared lifecycle phases for tracker-neutral orchestration.
- Backend route-policy validation wins over prompt prose; do not perform backend-owned pre-dispatch transitions from prompt text.
...
## TAPD Reconciliation Lifecycle

<!-- symphony-include: _partials/tracker/tapd_reconciliation_lifecycle.md -->
