...
agent_runtime:
  placement: local
workspace:
  # Set this before running the canary, for example:
  # export SYMPHONY_OPENCODE_CANARY_WORKSPACE_ROOT="$HOME/code/symphony-opencode-canary-workspaces"
  root: $SYMPHONY_OPENCODE_CANARY_WORKSPACE_ROOT
...
wrappers. For routine Linear tracker actions, repo-core actions, and
repo-provider change-proposal actions, use the exact runtime tool names listed
in the generated inventory below. If a required typed tool is missing, stop as
blocked, record the blocker in the workpad when workpad tooling is available,
and follow workflow-defined blocker handling. Do not ask a human for interactive
...
5. For `Todo` tickets, do startup sequencing in this exact order:
   ...
   - Find or create the `## OpenCode Workpad` bootstrap comment through inventory-listed typed tracker tools.
...
1.  Find or create a single persistent workpad comment for the issue:
    ...
    - Search snapshot comments for a marker header: `## OpenCode Workpad`.
...
- Use exactly one persistent workpad comment (`## OpenCode Workpad`) per issue, and update it through the inventory `tracker.upsert_workpad` typed tool.
