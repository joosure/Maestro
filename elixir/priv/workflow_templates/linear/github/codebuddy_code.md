...
from the generated inventory, stop as blocked, record the blocker in the
...
5. For `Todo` tickets, do startup sequencing in this exact order:
   ...
   - Find or create the `## CodeBuddy Code Workpad` bootstrap comment through inventory-listed typed tracker tools.
...
1.  Find or create a single persistent workpad comment for the issue:
    ...
    - Search snapshot comments for a marker header: `## CodeBuddy Code Workpad`.
...
4. Reset the existing `## CodeBuddy Code Workpad` comment through the inventory `tracker.upsert_workpad` typed tool; do not delete the comment or create a parallel workpad.
...
6. Start over from the normal kickoff flow:
   ...
   - Rewrite the same `## CodeBuddy Code Workpad` comment with a fresh plan/checklist.
...
- Use exactly one persistent workpad comment (`## CodeBuddy Code Workpad`) per issue, and update it through the inventory `tracker.upsert_workpad` typed tool.
