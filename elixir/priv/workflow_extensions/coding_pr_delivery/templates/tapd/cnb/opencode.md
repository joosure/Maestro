...
## Provider Runtime: OpenCode Message Dynamic-Tool Bridge

OpenCode receives Symphony Dynamic Tools through the runtime bridge configured
by Symphony for this app-server session. The configured provider process is
started with `opencode serve`, and prompts are posted through OpenCode's
synchronous `/session/:id/message` endpoint. For routine TAPD tracker actions,
repo-core actions, and CNB repo-provider change-proposal actions, use the exact
provider-facing callable names listed in the generated inventory below. The
inventory is the source for provider-specific callable names; the bundled skills
define typed capability semantics and argument shape.
If a required typed tool is missing, stop as blocked, record the blocker in the
TAPD workpad when workpad tooling is available, and follow workflow-defined
blocker handling. Do not ask a human for interactive setup during the session.
...
- The Open Code CLI must be available as `opencode`. If `agent_provider.options.credential_ref` is configured, that credential reference must be present in the Symphony agent credentials store; otherwise Open Code uses its normal local provider credentials.
