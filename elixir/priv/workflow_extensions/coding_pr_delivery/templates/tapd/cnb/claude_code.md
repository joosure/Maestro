...
prerequisite whenever this session exposes Dynamic Tools; it is not a TAPD/CNB
workflow prerequisite. For routine TAPD tracker actions, repo-core actions, and
CNB repo-provider change-proposal actions, use the exact provider-facing
callable names listed in the generated inventory below. The inventory is the
source for provider-specific callable names; the bundled skills define typed
capability semantics and argument shape. If a required typed tool is missing,
stop as blocked, record the blocker in the TAPD workpad when workpad tooling is
