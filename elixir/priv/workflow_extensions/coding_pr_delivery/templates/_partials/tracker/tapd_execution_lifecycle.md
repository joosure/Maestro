...
1. From `repo/`, determine current repo state through inventory-listed repo-core typed tools when they cover the needed facts. Use `"${SYMPHONY_WORKSPACE_AUTOMATION_DIR}/bin/repo"` only as explicitly documented helper path for repo-core facts not exposed by the inventory.
...
2. Create or update the PR through the inventory `repo.create_or_update_change_proposal` typed tool when it is listed. Confirm the resulting PR with `repo.change_proposal_snapshot` when it is listed.
   - Use the inventory `tracker.attach_external_reference` typed tool with `reference_kind: "change_proposal"` to attach the PR URL. TAPD currently exposes this to reviewers through the canonical workpad comment, while backend evidence comes from typed-tool results and KnownTarget registration.
...
4. Update the persistent workpad with final checklist status and validation notes.
   ...
   - This template guides handoff order only. Backend readiness remains authoritative; if a move tool returns missing or stale readiness reasons, refresh the named evidence and retry instead of bypassing the guard.
