Use this lifecycle for TAPD workflows where Coding PR Delivery reconciliation
owns the transition from review to merge, and where `rework`
...
4. If approved and PR checks/mergeability are ready, Coding PR Delivery reconciliation moves the story to `{{ issue.workflow.raw_state_by_route_key.merging }}`. A human may also move it there manually when the local process requires a manual tracker transition.
