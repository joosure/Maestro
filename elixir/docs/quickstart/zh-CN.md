...
| `change proposal reconciliation` | 在评审阶段自动核对已关联 PR/MR 是否满足 checks、可合并等 gate 的后台处理 |
...
请同时在 TAPD 的流程 / 工作流配置页面配置上面推荐的流转关系，而不只是创建这些状态显示名。Quickstart 路径要求状态显示名与上表完全一致，且状态之间允许上面推荐的流转；否则 `../scripts/tapd-workflow-init` 会报错并提示回到 TAPD 工作流配置页面修正。`需求池` 是人工入口，不交给 Maestro 自动扫描；`待开发` 是 Maestro 自动化闭环的入口。
