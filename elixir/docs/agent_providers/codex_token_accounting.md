...
This document explains how Codex reports token usage through the app-server protocol and how Maestro should account for it.

It complements:

- [`../architecture.md`](../architecture.md): module placement and ownership boundaries for provider-related implementation changes
- [`../logging.md`](../logging.md): provider-neutral observability conventions
- [`./codex.md`](./codex.md): Codex provider runtime, configuration, automation, and logging notes
- [`../../README.md`](../../README.md): operator-facing runtime behavior and implementation overview
...
Maestro sees these events wrapped inside the app-server message stream.
...
For Maestro, `model_context_window` should be displayed or logged separately from spend.

## Recommended Accounting Strategy For Maestro
...
3. `turn/completed.usage`, only when no cumulative thread/token-count total is present

Ignore these for accounting:

- `thread/tokenUsage/updated.tokenUsage.last`
- `TokenCountEvent.info.last_token_usage`
- generic `usage` maps on events other than `turn/completed`
...
When `turn/completed.usage` is used, treat it as a backup snapshot for missing cumulative totals,
not as a fresh additive increment.
...
- If the event is `turn/completed` and carries an integer token `usage` map, treat that usage as a
  backup high-water snapshot.
- Otherwise ignore the event for accounting and keep the last accepted high-water mark unchanged.
...
## What Maestro Should And Should Not Do

### Do

- Prefer `thread/tokenUsage/updated` for live reporting.
- Treat `tokenUsage.total` as authoritative for thread totals.
- Use `turn/completed.usage` only as a backup source when no cumulative live/thread total is present.
- Key accounting by `thread_id`, not just issue id.
- Expect one thread to span multiple turns when Maestro reuses a live Codex thread.

### Do not

- Do not treat every `usage` map as absolute.
- Do not count `tokenUsage.last` or `last_token_usage` into dashboard totals.
- Do not add turn-completed `usage` on top of already-counted live thread totals; use it only as
  the no-cumulative-total backup path.
- Do not reset accounting just because a new turn starts on the same thread.

## Practical Interpretation For Maestro Logs

When reading sanitized app-server events or internal test fixtures:
...
That is a strong signal for Maestro:

- use absolute totals as the main accounting surface
- ignore last/delta values for totals

## Recommended Maestro Documentation Contract

If Maestro documents token reporting externally, the contract should be:

- Live token totals come from Codex thread-scoped cumulative usage.
- Incremental usage may also be emitted, but Maestro does not use it for totals.
- Turn-completed usage is event-specific; Maestro may use it as a backup snapshot but never as a
  fresh additive increment on top of live thread totals.
...
- Use backup source `info.total_token_usage`
- Use backup source `turn/completed.usage` only when no cumulative total is available
