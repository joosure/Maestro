defmodule Mix.Tasks.Workflow.CommandTest do
  ...
  alias Mix.Tasks.Workflow.Command, as: WorkflowCommandTask
  ...
  test "prints help" do
    output = capture_io(fn -> WorkflowCommandTask.run(["--help"]) end)

    assert output =~ "mix workflow.command"
    assert output =~ "--id <command-id>"
    assert output =~ "-- [command args]"
  end

  test "rejects missing issue id" do
    assert_raise Mix.Error, ~r/--issue is required/, fn ->
      capture_io(fn ->
        WorkflowCommandTask.run(["--id", @command_id, "--", "--json"])
      end)
    end
  end

  test "rejects missing command id" do
    assert_raise Mix.Error, ~r/--id or command id is required/, fn ->
      capture_io(fn ->
        WorkflowCommandTask.run(["--", "--json"])
      end)
    end
  end
  ...
  test "runs dry-run one-shot reconciliation as JSON" do
    ...
    output =
      capture_io(fn ->
        WorkflowCommandTask.run(["--id", @command_id, "--", "--issue", "issue-task-dry-run", "--json"])
      end)
