defmodule SymphonyElixir.Workflow.StructuredExecutionPlan.WorkpadRendererTest do
  ...
  test "renders deterministic Markdown for the same canonical plan" do
    ...
    assert {:ok, first} = WorkpadRenderer.render(plan)
    assert {:ok, second} = WorkpadRenderer.render(plan)
  ...
  test "checkbox state comes only from item status" do
    ...
    assert {:ok, %{"body" => body}} = WorkpadRenderer.render(plan)
