defmodule SymphonyElixir.Workflow.StructuredExecutionPlan.WorkpadWriterTest do
  ...
  test "default gate-off behavior skips without touching store or tracker" do
    ...
    assert {:ok,
            ...
            }} = WorkpadWriter.write("missing-plan", tracker_executor: executor)
  end
  ...
  test "missing rendered Workpad is created through tracker upsert and marker is recorded", %{store: store} do
    ...
    assert {:ok,
            %{
             ...
             WorkpadWriter.write("plan-test-1",
               gates: @gates,
               server: store,
               tracker_comments: [],
  ...
  test "known rendered Workpad updates by workpad id without inspecting tracker comment bodies", %{store: store} do
    ...
    assert {:ok, old_render} = WorkpadRenderer.render(base_plan, mode: "write")
