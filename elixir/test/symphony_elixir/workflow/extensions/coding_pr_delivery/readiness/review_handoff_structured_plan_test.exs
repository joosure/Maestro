defmodule SymphonyElixir.Workflow.Extensions.CodingPrDelivery.Readiness.ReviewHandoffStructuredPlanTest do
  ...
  alias SymphonyElixir.Workflow.Extensions.CodingPrDelivery.Readiness.StructuredPlanReviewHandoff.Plan.Reader
  ...
  @review_handoff_gate Contract.transition_readiness_required_gate_key()
  ...
  test "structured plan checks fail closed on non-keyword opts" do
    assert [check] = StructuredPlanReviewHandoff.checks(@workflow, issue(), ready_evidence()["observations"], [:invalid_opts])

    assert check["status"] == "failed"
    assert check["reason_code"] == "structured_plan_options_invalid"
    refute inspect(check) =~ "invalid_opts"
  end

  test "structured plan reader port fails closed on non-keyword opts" do
    assert {:error, reason} = Reader.fetch(%{}, %{}, [:invalid_opts])

    assert reason.code == "store_unavailable"
    assert reason.reason == "structured_plan_reader_backend_invalid"
    assert reason.value_type == "non_keyword_list"
    refute inspect(reason) =~ "invalid_opts"
  end
  ...
  test "reader backend opts own structured plan store injection", %{store: store} do
    create_plan!(store, complete_plan())

    opts =
      store
      |> gated_opts()
      |> Keyword.update!(:structured_execution_plan, &Map.delete(&1, :server))
      |> Keyword.put(:structured_plan_reader_opts, server: store)

    assert :ok = ReviewHandoff.validate(@workflow, issue(), opts)
  end
