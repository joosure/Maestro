...
GitHub, CNB, Codex, Claude Code, CodeBuddy Code, OpenCode, or external
credentials:
...
Running `mise exec -- ./bin/symphony` without a workflow path or `--template` loads
...
`linear/github/codebuddy_code`, `linear/github/opencode`,
`tapd/github/codex`, `tapd/cnb/codebuddy_code`, and `tapd/cnb/opencode`.
...
Claude Code, CodeBuddy Code, or OpenCode backed templates.
...
explicit override. `SOURCE_REPO_PROVIDER_REQUIRED_PR_LABEL` is optional and only
