...
agent_provider:
  kind: codex
  options:
    command: codex --config shell_environment_policy.inherit=all --config model_reasoning_effort=xhigh --config 'project_root_markers=[]' --model gpt-5.3-codex app-server
...
## Prerequisite: Codex dynamic tools are available

Codex receives Symphony Dynamic Tools through the runtime MCP bridge configured
by Symphony at session startup.
For routine Linear tracker actions, repo-core actions, and repo-provider
change-proposal actions, use the exact provider-facing callable tool names
listed in the generated inventory below. If a required typed tool is missing,
stop as blocked, record the blocker in the workpad when workpad tooling is
...
5. For `Todo` tickets, do startup sequencing in this exact order:
   ...
   - Find or create the `## Codex Workpad` bootstrap comment through inventory-listed typed tracker tools.
...
1.  Find or create a single persistent workpad comment for the issue:
    ...
    - Search snapshot comments for a marker header: `## Codex Workpad`.
...
6. Start over from the normal kickoff flow:
   ...
   - Rewrite the same `## Codex Workpad` comment with a fresh plan/checklist.
...
- Use exactly one persistent workpad comment (`## Codex Workpad`) per issue, and update it through the inventory `tracker.upsert_workpad` typed tool.
