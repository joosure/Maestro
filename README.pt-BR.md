...
## Uma plataforma de orquestração e governança para agentes de IA na engenharia empresarial.

Maestro é uma plataforma de orquestração e governança de agentes de IA para equipes de engenharia. Ele conecta issue trackers, requisitos, repositórios, Agent Providers, ambientes de runtime, integrações de ferramentas e evidence de entrega para que Codex, Claude Code, CodeBuddy Code, OpenCode e futuros Coding Agents possam assumir trabalho real de projeto, executá-lo, enviar mudanças e deixar evidence auditável.

Não é mais um coding agent.

Ele permite que Codex, Claude Code, CodeBuddy Code, OpenCode e futuros agentes trabalhem a partir de sistemas reais de projeto, repositórios reais, workflows reais e restrições operacionais reais.

> **Symphony provou o padrão. Maestro constrói a plataforma.**

---

## Início rápido com contêineres

Execute o workflow local memory/mock sem credenciais externas:

```bash
docker compose -f deploy/compose/compose.quickstart.yml up --build
```

Abra o dashboard em `http://localhost:4000`.

Para o quickstart completo com contêineres, integração de workflows reais, volumes e tratamento de credenciais, consulte [`docs/deployment/container.md`](./docs/deployment/container.md).
...
Ele generaliza a implementação de referência original `Linear + Codex` em uma **plataforma de orquestração e governança tracker-driven e provider-neutral** para workflows modernos de engenharia.
...
| Agent Provider | Codex, Claude Code, CodeBuddy Code, OpenCode e providers extensíveis para futuros agentes CLI ou remotos |
...
### 1. Trackers são a superfície de dispatch
...
Codex, Claude Code, CodeBuddy Code, OpenCode e futuros agentes são tratados como providers substituíveis. Maestro padroniza o lifecycle de que a camada de orquestração precisa: session creation, turn execution, tool-call capture, evidence collection, quota awareness e cleanup.
...
Maestro fica mais claro como uma arquitetura de orquestração e governança de engenharia: o orchestrator mantém o estado de scheduling, a workflow policy mantém o significado de negócio e os providers entram por contratos explícitos. Governance e evidence atravessam todo o run.
...
A arquitetura de plataforma do Maestro pode ser resumida em oito camadas. As seis primeiras descrevem o caminho principal de execução; governance e evidence são camadas transversais de controle e auditoria.

| Camada | Componentes | Responsabilidade |
| --- | --- | --- |
| Work Source Layer | Tracker / Issue / Story / Ticket | Define por onde o trabalho entra no sistema |
| Workflow Policy Layer | `WORKFLOW.md` / Workflow Profile / Route Policy / Capabilities / Human Gate Declarations | Define como o time quer que o trabalho do agent avance |
| Control Plane Layer | Orchestrator / Scheduler / Dispatch / Retry / Reconciliation / State Tracking | Decide quando o trabalho roda, tenta novamente, para ou reconcilia |
| Execution Runtime Layer | Agent Runner / Workspace / Runtime / Local / SSH / Worker Daemon / Session Lifecycle | Executa cada work item em um boundary isolado |
| Agent Provider Layer | Codex / Claude Code / CodeBuddy Code / OpenCode / Mock / Future Agent Providers | Envolve implementações de agent substituíveis com um lifecycle comum |
| Provider & Tool Integration Layer | Dynamic Tool Bridge / Tracker Adapter / Repo Facade / Repo Provider | Conecta sistemas externos por contratos provider-neutral |
| Governance Layer | Credential / Lease / Quota / Redaction / Approval / Policy Enforcement | Controla acesso, capacidade, approvals e postura de segurança |
| Evidence & Observability Layer | Events / JSON Logs / Diff / PR / CI / Evidence / Audit Trail / Dashboard | Registra o que aconteceu, por que aconteceu e se o resultado é confiável |
...
| `Agent Provider` | Start, drive, stream, stop e cleanup de sessões Codex / Claude Code / CodeBuddy Code / OpenCode / Mock |
...
Os valores de agent provider kind são strings canônicos de runtime. Os built-ins atuais são `codex`, `claude_code`, `codebuddy_code`, `opencode` e `mock`; aliases suportados são normalizados pelo owner de provider-kind em Elixir antes da consulta ao registry.

Os strings canônicos de tracker, repo-provider e agent-provider pertencem aos módulos Elixir `Tracker.Kinds`, `RepoProvider.Kinds` e `AgentProvider.Kinds`, mantendo registries, defaults e documentação alinhados.
...
Se esta é a sua primeira vez executando o Maestro, comece pelo [Newcomer End-to-End Run Guide](./elixir/docs/quickstart/en.md). O guia cobre o fluxo local `memory/no_repo/mock`, o fluxo real `TAPD + CNB + CodeBuddy Code` e o fluxo real `Linear + GitHub + OpenCode`.
...
Isso inicia o serviço com o template `memory/no_repo/mock` e expõe o dashboard/API opcional em `http://localhost:4000`. Ele usa o tracker em memória, o repo provider em memória e o mock agent provider, então não precisa de credenciais Linear, GitHub, Codex, Claude Code, CodeBuddy Code, OpenCode ou CNB.
...
- Use fake ou simulated agent adapters apenas em testes ou trabalho de extensão pelo adapter registry; os agent providers integrados são `codex`, `claude_code`, `codebuddy_code` e `opencode`.
...
- engineering teams avaliando Codex, Claude Code, CodeBuddy Code, OpenCode ou futuros coding agents;
