...
API exposes the same metadata at `/api/v1/worker-daemon/source`. Configure the
public source location for a deployment with:
...
For compatibility with early Symphony-derived deployments, the dashboard also
accepts `SYMPHONY_SOURCE_URL` and `SYMPHONY_SOURCE_REVISION` as fallback
