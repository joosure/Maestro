...
## Una plataforma de orquestación y gobernanza para agentes de IA en ingeniería empresarial.

Maestro es una plataforma de orquestación y gobernanza de agentes de IA para equipos de ingeniería. Conecta issue trackers, requisitos, repositorios, Agent Providers, entornos de runtime, integraciones de herramientas y evidence de entrega para que Codex, Claude Code, CodeBuddy Code, OpenCode y futuros Coding Agents puedan asumir trabajo real de proyecto, ejecutarlo, enviar cambios y dejar evidence auditable.

No es otro coding agent.

Permite que Codex, Claude Code, CodeBuddy Code, OpenCode y futuros agentes trabajen desde sistemas reales de proyecto, repositorios reales, workflows reales y restricciones operativas reales.
...
Ejecuta el workflow local memory/mock sin credenciales externas:

```bash
docker compose -f deploy/compose/compose.quickstart.yml up --build
```

Abre el dashboard en `http://localhost:4000`.

Para el quickstart completo con contenedores, integración de workflows reales, volúmenes y manejo de credenciales, consulta [`docs/deployment/container.md`](./docs/deployment/container.md).
...
Generaliza la implementación de referencia original `Linear + Codex` en una **plataforma de orquestación y gobernanza tracker-driven y provider-neutral** para workflows modernos de ingeniería.
...
| Agent Provider | Codex, Claude Code, CodeBuddy Code, OpenCode y providers extensibles para futuros agentes CLI o remotos |
...
Codex, Claude Code, CodeBuddy Code, OpenCode y futuros agentes se tratan como providers reemplazables. Maestro estandariza el lifecycle que necesita la capa de orquestación: session creation, turn execution, tool-call capture, evidence collection, quota awareness y cleanup.
...
Maestro se entiende mejor como una arquitectura de orquestación y gobernanza de ingeniería: el orchestrator conserva el estado de scheduling, la workflow policy conserva el significado de negocio y los providers entran mediante contratos explícitos. Governance y evidence atraviesan todo el run.
...
### Capas centrales de arquitectura

La arquitectura de plataforma de Maestro se resume en ocho capas. Las primeras seis describen la ruta principal de ejecución; governance y evidence son capas transversales de control y auditoría.

| Capa | Componentes | Responsabilidad |
| --- | --- | --- |
| Work Source Layer | Tracker / Issue / Story / Ticket | Define por dónde entra el trabajo al sistema |
| Workflow Policy Layer | `WORKFLOW.md` / Workflow Profile / Route Policy / Capabilities / Human Gate Declarations | Define cómo quiere el equipo que avance el trabajo del agent |
| Control Plane Layer | Orchestrator / Scheduler / Dispatch / Retry / Reconciliation / State Tracking | Decide cuándo se ejecuta, reintenta, detiene o reconcilia el trabajo |
| Execution Runtime Layer | Agent Runner / Workspace / Runtime / Local / SSH / Worker Daemon / Session Lifecycle | Ejecuta cada work item dentro de un boundary aislado |
| Agent Provider Layer | Codex / Claude Code / CodeBuddy Code / OpenCode / Mock / Future Agent Providers | Envuelve implementaciones de agent reemplazables con un lifecycle común |
| Provider & Tool Integration Layer | Dynamic Tool Bridge / Tracker Adapter / Repo Facade / Repo Provider | Conecta sistemas externos mediante contratos provider-neutral |
| Governance Layer | Credential / Lease / Quota / Redaction / Approval / Policy Enforcement | Controla acceso, capacidad, approvals y postura de seguridad |
| Evidence & Observability Layer | Events / JSON Logs / Diff / PR / CI / Evidence / Audit Trail / Dashboard | Registra qué pasó, por qué pasó y si el resultado es confiable |
...
| `Agent Provider` | Start, drive, stream, stop y cleanup de sesiones Codex / Claude Code / CodeBuddy Code / OpenCode / Mock |
...
Los valores de agent provider kind son strings canónicos de runtime. Los built-ins actuales son `codex`, `claude_code`, `codebuddy_code`, `opencode` y `mock`; los aliases soportados se normalizan por el owner de provider-kind en Elixir antes de consultar el registry.

Los strings canónicos de tracker, repo-provider y agent-provider pertenecen a los módulos Elixir `Tracker.Kinds`, `RepoProvider.Kinds` y `AgentProvider.Kinds`, para mantener alineados registries, defaults y documentación.
...
Si es la primera vez que ejecutas Maestro, empieza con la [Newcomer End-to-End Run Guide](./elixir/docs/quickstart/en.md). La guía cubre el flujo local `memory/no_repo/mock`, el flujo real `TAPD + CNB + CodeBuddy Code` y el flujo real `Linear + GitHub + OpenCode`.
...
Esto inicia el servicio con el template `memory/no_repo/mock` y expone el dashboard/API opcional en `http://localhost:4000`. Usa el tracker en memoria, el repo provider en memoria y el mock agent provider, por lo que no requiere credenciales de Linear, GitHub, Codex, Claude Code, CodeBuddy Code, OpenCode ni CNB.
...
- Usa fake o simulated agent adapters solo en tests o trabajo de extensión mediante el adapter registry; los agent providers integrados son `codex`, `claude_code`, `codebuddy_code` y `opencode`.
...
- engineering teams que evalúan Codex, Claude Code, CodeBuddy Code, OpenCode o futuros coding agents;
