...
## Autonomous engineering agent를 위한 control plane.

Maestro는 issue tracker를 AI agent의 실행 레이어로 바꿉니다. 작업 dispatch, runtime 관리, provider 조율, evidence 추적을 담당하고, agentic engineering을 팀 규모에서 운영 가능하게 만듭니다.

또 하나의 coding agent가 아닙니다.

Codex, Claude Code, OpenCode, 그리고 미래의 agent가 실제 project system, repository, workflow, 운영 제약 안에서 일할 수 있게 하는 orchestration platform입니다.
...
원래의 `Linear + Codex` reference implementation을 현대 engineering workflow를 위한 **tracker-driven, provider-neutral orchestration platform**으로 일반화합니다.
...
| Layer | Maestro가 제공하는 것 |
| --- | --- |
| Tracker | Linear, TAPD, Memory, 그리고 Jira, YouTrack, Feishu Project, GitHub Issues 등으로 확장 가능한 adapter |
| Agent Provider | Codex, Claude Code, OpenCode, 그리고 미래 CLI 또는 remote agent provider |
...
Codex, Claude Code, OpenCode, 미래 agent는 replaceable provider로 다뤄집니다. Maestro는 orchestration layer가 필요로 하는 lifecycle, 즉 session creation, turn execution, tool-call capture, evidence collection, quota awareness, cleanup을 표준화합니다.
...
| `Agent Provider` | Codex / Claude Code / OpenCode / Mock session start, drive, stream, stop, cleanup |
...
이 명령은 `memory/no_repo/mock` template으로 service를 시작하고 선택적 dashboard/API를 `http://localhost:4000`에 노출합니다. Memory tracker, memory repo provider, mock agent provider를 사용하므로 Linear, GitHub, Codex, Claude Code, OpenCode, CNB credentials가 필요 없습니다.
...
- fake 또는 simulated agent adapter는 adapter registry를 통해 tests나 extension work에서만 사용하세요. 내장 agent providers는 `codex`, `claude_code`, `opencode`입니다.
...
- Codex, Claude Code, OpenCode, 미래 coding agents를 평가하는 engineering teams;
