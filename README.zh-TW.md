...
Maestro 會把你的 issue tracker 變成 AI Agent 的執行層，負責派發工作、管理 runtime、協調 provider、追蹤 evidence，並讓 agentic engineering 能在團隊規模下可營運。

它不是另一個 coding agent。

它是讓 Codex、Claude Code、OpenCode 和未來 Agent 從真實專案系統、真實倉庫、真實 workflow 與真實營運限制中工作的編排平台。
...
它把原始的 `Linear + Codex` reference implementation 泛化為一個 **tracker-driven、provider-neutral 的現代工程 workflow 編排平台**。
...
| Agent Provider | Codex、Claude Code、OpenCode，並可擴展未來 CLI 或 remote agent |
...
Codex、Claude Code、OpenCode 和未來 agent 都被視為可替換 provider。Maestro 標準化編排層真正需要的 lifecycle：session creation、turn execution、tool-call capture、evidence collection、quota awareness 和 cleanup。
...
| `Agent Provider` | start、drive、stream、stop 和清理 Codex / Claude Code / OpenCode / Mock session |
...
Maestro 不限於「從 issue 寫程式碼」。它可以用同一個平台層編排多種工程 workflow。
...
這會使用 `memory/no_repo/mock` template 啟動服務，並在 `http://localhost:4000` 暴露可選 dashboard/API。它使用記憶體 tracker、記憶體 repo provider 和 mock agent provider，不需要 Linear、GitHub、Codex、Claude Code、OpenCode 或 CNB 憑證。
...
- 需要無外部憑證驗證編排流程時，配置 `tracker.kind: memory` 和 `repo.provider.kind: memory`。
- fake/simulated agent adapter 只透過 adapter registry 用於測試或擴展開發；目前內建 agent provider 是 `codex`、`claude_code` 和 `opencode`。
...
- 正在評估 Codex、Claude Code、OpenCode 或未來 coding agents 的 engineering teams；
