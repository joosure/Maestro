...
## 面向企业级工程的 AI Agent 运营与治理平台。

Maestro 是一个面向工程团队的 AI Agent 运营与治理平台。它把工单系统、需求文档、代码仓库、Agent Provider、运行环境和交付证据连接起来，让 Codex、Claude Code、CodeBuddy Code、OpenCode 等 Coding Agent 可以从真实项目任务中自动接单、执行、提交和留痕。
...
运行本地 memory/mock 工作流，不需要任何外部凭据：

```bash
docker compose -f deploy/compose/compose.quickstart.yml up --build
```

打开 dashboard：`http://localhost:4000`。

完整容器 Mock Quickstart、真实 workflow 容器集成、数据卷和凭据处理请参考 [`docs/deployment/container.zh-CN.md`](./docs/deployment/container.zh-CN.md)。
...
它把原始的 `Linear + Codex` 参考实现，泛化为一个 **由 Tracker 驱动、对 Provider 中立、面向真实工程流程的 AI Agent 运营与治理平台**。
...
| Agent Provider | Codex、Claude Code、CodeBuddy Code、OpenCode，并可扩展未来 CLI Agent 或远端 Agent 服务 |
...
Codex、Claude Code、CodeBuddy Code、OpenCode 和未来更多 Agent 都被视为可替换 Provider。Maestro 标准化平台层真正需要的生命周期：创建 session、执行 turn、捕获 tool call、收集 evidence、感知 quota、清理运行状态。
...
Maestro 更适合被理解为一套工程运营与治理架构：
Orchestrator 持有调度状态，workflow policy 持有业务语义，provider 通过明确 contract 接入。
Governance 和 evidence 贯穿整个运行过程。
...
Maestro 的平台级核心架构可以归纳为八层。前六层构成主要执行路径；
Governance 和 Evidence & Observability 是贯穿全流程的控制与审计层。

| 层级 | 组成 | 职责 |
| --- | --- | --- |
| Work Source Layer | Tracker / Issue / Story / Ticket | 定义工作从哪里进入系统 |
| Workflow Policy Layer | `WORKFLOW.md` / Workflow Profile / Route Policy / Capabilities / Human Gate Declarations | 定义团队希望 Agent 如何执行工作 |
| Control Plane Layer / 调度控制层 | Orchestrator / Scheduler / Dispatch / Retry / Reconciliation / State Tracking | 决定任务何时运行、重试、停止或对账 |
| Execution Runtime Layer | Agent Runner / Workspace / Runtime / Local / SSH / Worker Daemon / Session Lifecycle | 在隔离执行边界中运行每个工作项 |
| Agent Provider Layer | Codex / Claude Code / CodeBuddy Code / OpenCode / Mock / Future Agent Providers | 用统一生命周期包装可替换的 Agent 实现 |
| Provider & Tool Integration Layer | Dynamic Tool Bridge / Tracker Adapter / Repo Facade / Repo Provider | 通过 provider-neutral contract 接入外部系统 |
| Governance Layer | Credential / Lease / Quota / Redaction / Approval / Policy Enforcement | 控制访问、容量、审批和安全策略 |
| Evidence & Observability Layer | Events / JSON Logs / Diff / PR / CI / Evidence / Audit Trail / Dashboard | 记录发生了什么、为什么发生、结果是否可信 |
...
| `Agent Provider` | 启动、驱动、流式输出、停止和清理 Codex / Claude Code / CodeBuddy Code / OpenCode / Mock session |
...
Maestro 不局限于“从 Issue 写代码”。它可以用同一套平台层承载多个工程流程。
...
`claude_code`、`codebuddy_code`、`opencode` 和 `mock`；支持的别名会先由 Elixir 的
...
如果你是第一次运行 Maestro，建议先按 [新人端到端运行指引（中文）](./elixir/docs/quickstart/zh-CN.md) 完成一遍。该指引覆盖本地 `memory/no_repo/mock`、真实 `TAPD + CNB + CodeBuddy Code`、真实 `Linear + GitHub + OpenCode` 三条路径，并包含环境变量、凭据登录、启动命令、验证方式和常见排障。

如果你只想用 Docker 快速体验 Dashboard 和 mock workflow，不想安装 Elixir、Erlang 或 `mise`，请先看 [容器 Mock Quickstart](./docs/deployment/container.zh-CN.md#容器-mock-quickstart)。
...
这会使用 `memory/no_repo/mock` template 启动服务，并在 `http://localhost:4000` 暴露可选 dashboard/API。它使用内存 Tracker、内存 repo provider 和 mock agent provider，不需要 Linear、GitHub、Codex、Claude Code、CodeBuddy Code、OpenCode 或 CNB 凭证。

要接入真实 tracker、代码仓库和 agent runtime，请按新人指引选择 `TAPD + CNB + CodeBuddy Code` 或 `Linear + GitHub + OpenCode`。以 Linear/GitHub/OpenCode 路径为例，请先配置所需凭据，再切换 template：
...
- 在需要无外部凭证验证执行流程时，配置 `tracker.kind: memory` 和 `repo.provider.kind: memory`。
- fake/simulated agent adapter 只通过 adapter registry 用于测试或扩展开发；当前内置真实 agent provider 包括 `codex`、`claude_code`、`codebuddy_code` 和 `opencode`。
...
- 正在评估 Codex、Claude Code、CodeBuddy Code、OpenCode 或未来 Coding Agent 的工程团队；
- 构建内部 AI 工程基础设施的平台团队；
- 设计 Agent Ops 工作流的 DevTools 团队；
- 希望让 Agent 基于现有 Tracker 工作的产品和研发组织；
- 研究 Agent 可靠性、证据链和工程运营体系的研究者；
...
Maestro 始于 [OpenAI Symphony](https://github.com/openai/symphony) 的 fork。原始 Symphony reference implementation 聚焦于 Linear 驱动的 Codex 自动执行。Maestro 将这一思路扩展为覆盖 Tracker、Agent Provider、Repo Provider、Workflow Profile、Runtime、Tool Bridge 和 Evidence 的平台架构。
